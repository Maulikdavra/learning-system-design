...
*A: A single-server ACID transaction is straightforward because one database engine controls the whole commit. Across shards, you need a distributed transaction protocol like two-phase commit which ensures that all participating shards either successfully commit the changes or collectively abort them, preventing partial updates that would violate data integrity, or you avoid distributed transactions entirely with patterns like the Saga pattern, where each shard does its own local transaction and compensating actions undo earlier steps if a later step fails. This is exactly why cross-shard transactions are called out as one of sharding's biggest challenges.*
